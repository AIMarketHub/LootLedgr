// LootLedger — Privacy Policy PDF/HTML export.
// Mirrors src/modals/AmlProgramPdf.jsx with paths swapped to
// privacy-policy defaults + cover-page wording reframed for a
// privacy policy artifact.
//
// Two outputs:
//   🖨 Print           — calls window.print(); the print stylesheet
//                        in src/index.css preserves the
//                        .receipt-print-area class.
//   ⬇ Download HTML   — extracts the live render and wraps it in
//                        a self-contained HTML5 document.
//                        Filename:
//                        Privacy-Policy-v{version}-{shopName}-{date}.html
...
export function PrivacyPolicyRender({version,shopName}){
  ...
  return <div className="receipt-print-area" style={{
    ...
    {/* Cover page — branded logo on white printable surface (DARK
    ...
    <div style={{textAlign:"center",padding:"40px 0",pageBreakAfter:"always"}}>
      ...
      <div style={{margin:"32px 0",fontSize:14}}>
        ...
        {data["s1.privacyOfficerName"]&&<div style={{marginTop:14}}>Privacy Officer: <strong>{sS(data["s1.privacyOfficerName"])}</strong></div>}
      </div>
      <div style={{fontSize:11,color:"#666",marginTop:24,padding:"12px",border:"1px solid #ccc"}}>
        This document describes how the entity above handles personal information for the purposes of the Privacy Act 1988 (Cth) and the Australian Privacy Principles. It also identifies the Privacy Officer, contact pathways, and complaint-handling procedures. Generated from LootLedger on {fmtDateTime(new Date().toISOString())}.
      </div>
    </div>
    ...
    <footer style={{textAlign:"center",fontSize:10,color:"#666",marginTop:24}}>
      <div>End of document. Privacy Officer contact details appear in Section 1.</div>
...
export default function PrivacyPolicyPdf({settings,pop,onClose}){
  ...
  if(!current){
    return <Modal title="📄 Privacy Policy — Export" onClose={onClose}>
  ...
  const onDownloadHtml=()=>{
    ...
    const filename="Privacy-Policy-v"+sS(current.version)+"-"+safeShopName(shopName)+"-"+today+".html";
    const html=
      '<!DOCTYPE html><html lang="en-AU"><head><meta charset="utf-8">'+
      '<title>Privacy Policy v'+sS(current.version)+' — '+sS(shopName)+'</title>'+
  ...
  return <Modal title={"📄 Privacy Policy v"+sS(current.version)+" — Export"} onClose={onClose} wide>
    ...
    <PrivacyPolicyRender version={current} shopName={shopName}/>
