// LootLedger — AML/CTF Program version history viewer.
// Phase 2.7 follow-up (2026-04-30). Lists every approved version
// in settings.amlProgram.versions[], lets the dealer drill into
// any version for a read-only view, and supports
// "Restore as new draft" — seeds settings.amlProgram.draft from
// a historical version's data so the dealer can edit and
// re-approve without losing the audit trail.
//
// Approved versions are immutable. The restore path always
// produces a NEW draft; the existing draft (if any) is replaced,
// and the user is warned before that happens.
...
// they see here against the form they would otherwise edit.
...
export default function AmlProgramHistory({settings,setSettings,activeStaff,pop,onClose,onRestoredOpenForm}){
  ...
  if(selectedVersion){
    return <Modal title={"📜 AML/CTF Program v"+sS(selectedVersion.version)} onClose={()=>setSelectedVersion(null)} wide>
  ...
  return <Modal title="📜 AML/CTF Program — Version History" onClose={onClose} wide>
