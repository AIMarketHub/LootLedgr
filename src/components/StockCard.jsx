  ...
  return <div style={{...c.card({padding:14}),marginBottom:10,borderLeft:"4px solid "+(s.policeHold?T.red:s.sold?T.muted:hoursLeft(s.holdUntil)>0?T.orange:T.readyGreen)}}>
    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:10}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:4}}>
        <button style={{...c.bsm(s.policeHold?T.redBg:T.border,s.policeHold?T.red:T.muted),padding:"6px 10px",fontSize:11}} onClick={()=>{if(s.policeHold){setPinModal({reason:"Remove police hold — Admin PIN required.",cb:()=>togglePoliceHold(s.id,false)});setPinVal("");}else togglePoliceHold(s.id,true);}}>{s.policeHold?"🚔 Held":"🚔 Hold"}</button>
