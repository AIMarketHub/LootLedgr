...
export default function Loot() {
  ...
  if(contrast!==0){
    const c=contrast; // -5 to +5
    const bAmt=Math.round(c*18); // border alpha shift
    const textShift=c>0?Math.max(0,Math.min(255,0+c*22)):0;
    const bgShift=c>0?Math.max(0,c*8):0;
    T={...T,
      border:c>0?"rgba(0,0,0,"+(0.12+c*0.075)+")":"rgba(0,0,0,"+(0.12+c*0.02)+")",
      borderHi:c>0?"rgba(0,0,0,"+(0.22+c*0.1)+")":"rgba(0,0,0,"+(0.22+c*0.02)+")",
      muted:c>0?"#"+Math.max(0,0x73-c*18).toString(16).padStart(2,"0").repeat(3):"#737373",
      text:c>0?"#000":"#"+Math.max(0x11,0x11+Math.round(c*8)).toString(16).padStart(2,"0").repeat(3),
      card:c>0?"#fff":"#"+Math.max(0xfa,0xff-Math.abs(c)*2).toString(16).padStart(2,"0").repeat(3),
      bg:c>0?"#dedad4":"#"+Math.max(0xee,0xf5-Math.abs(c)*3).toString(16).padStart(2,"0"),
      gold:c>0?"#7a5200":"#9C7A00",
    };
  }
            ...
            {settingsOpen.appearance&&(
              <div style={{paddingBottom:14}}>
                ...
                <div style={{marginBottom:16}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:T.muted,marginTop:4}}>
                    <span>Thin</span>
                    <span style={{color:T.gold,fontWeight:"bold",fontWeight:fontWeight}}>{fontWeight===400?"Regular":fontWeight<400?"Light":fontWeight===500?"Medium":fontWeight===600?"Semi-Bold":"Bold"}</span>
                    <span>Bold</span>
                  </div>
                </div>
                <div style={{marginBottom:16}}>
