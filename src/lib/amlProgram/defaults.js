...
export function buildDefaults(settings){
  ...
  return{
    ...
    "s5.structuringDetection":"LootLedger evaluates a rolling 30-day cash buy total per customer at the Compliance step of every cash transaction. The check uses the linked client record where one exists; for transactions that haven't yet been linked, a fuzzy fallback aggregates by customer name. The threshold mirrors the AUSTRAC TTR threshold ($10,000 AUD). At 80% of threshold the system raises a yellow staff-only banner advising review; at 100% the banner becomes a hard block — staff cannot advance without entering the Admin PIN and a written justification of at least 20 characters. The override flag and reason persist on the transaction record for the 7-year audit trail. Caveat: the current data model uses a single payment-method per transaction, so cash-out (we pay vendor) is captured but cash-in sales (customer pays cash to buy from stock) are not currently aggregated; this limitation is being tracked.\n\nWhere a structuring pattern is detected the AML/CTF Compliance Officer reviews the override justifications weekly and decides whether to file an SMR with AUSTRAC. SMR filings are confidential under section 123 of the AML/CTF Act — staff are explicitly trained never to disclose to a customer that the structuring banner appeared, that an override was applied, or that an SMR has been or may be filed.",
    "s5.linkedTxDetection":"At the Client step of every transaction, LootLedger checks for prior transactions by the same customer earlier on the same calendar day (linked by clientId where the customer has a persisted record, by name fallback otherwise). When prior same-day transactions exist, an informational banner surfaces the count, the time of the most recent prior transaction, and a click-through to a read-only summary modal. The banner is non-blocking — staff acknowledge it and proceed — but the visibility ensures coordinated theft, structuring patterns, or returning-customer fraud are noticed at the moment of the next transaction rather than after the fact. The banner is staff-only; it does not surface on receipts or external integrations.",
...
export const FIELD_META={
  ...
  "s5.linkedTxDetection":{type:"textarea",label:"Linked-transaction detection (same-day)"},
  // s6
...
export const SECTION_FIELDS={
  ...
  s5:["s5.monitoringProcedure","s5.suspiciousIndicators","s5.smrResponsible","s5.smrSubmissionMethod","s5.tippingOff","s5.ttrTrigger","s5.ttrAggregation","s5.ttrResponsible","s5.iftiProcedure","s5.structuringDetection","s5.linkedTxDetection"],
